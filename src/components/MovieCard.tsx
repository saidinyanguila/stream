...
const MovieCard = ({ item }: MovieItem) => {
    ...
    return (
        <a href={`/stream/watch/movie/${item.id}`} className="relative text-white shrink-0 md:w-[12%] md:min-w-[175px] min-w-[135px] cursor-pointer">
            <div className="relative relative w-full aspect-[7/10] rounded-md overflow-hidden">
                <img className="w-full h-full" src={`https://image.tmdb.org/t/p/w500${item.poster_path}`} alt={`${item.title} Cover Art`}/>
            ...
            <div className="py-2 px-1">
                <p className="text-white/100 text-[15px] font-[500]">{item.title}</p>
                <p className="text-white/60 text-[13px] ">{item.release_date.split("-")[0]}</p>
            </div>
        </a>
    )
}
